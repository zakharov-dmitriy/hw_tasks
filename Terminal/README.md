...
|36|Команда в одну строку. Переместить в любую одну папку текстовые файлы, у которых в содержимом есть слово *“sec”*|`find -type f -name '*' -exec grep -rlw 'sec' {} + \| xargs -I{} mv {} dir_2`|
|37|Команда в одну строку. Скопировать в любую одну папку текстовые файлы, у которых в содержимом есть слово *“sec”*|`find -type f -name '*' -exec grep -rwl 'sec' {} \; -exec cp -t . {} +`|
...
Флаг `-t` в команде `cp` используется для указания целевой директории, куда нужно скопировать файлы.
